...
void Cmd_Info_Boot(gentity_t *ent)
{
	...
	char *s3 = "\n^7Simply click this button to block, and face the attack properly. You can also chain with red style.\n";
	char *s4 = "\n^7For the full experience, ^3DOWNLOAD THE CLIENTSIDE! ^7If you haven't already, type:\n^3sv_allowDownload 1^7. Then ^3/reconnect^7.\n";
	char *s5 = "\n\n^7To view all commands, type ss_, then hit tab. To view all features, type ^3/features^7.\n";

	trap_SendServerCommand(ent - g_entities, va("print \"%s%s%s%s%s\"", s1, s2, s3, s4, s5));
}

void Cmd_Features_Boot(gentity_t *ent)
{
	char *s1 = "\n\n               ^1** ^7Features^1** \n\n";
	char *s2 = "\n^1- ^7Manual blocking.\n";
	char *s3 = "\n^1- ^7Cancelling attacks.\n";
	char *s4 = "\n^1- ^7Location based damage (head, body, feet).\n";
	char *s5 = "\n^1- ^7Red style chains.\n";
	char *s6 = "\n^1- ^7Slower but more powerful guns.\n";
	char *s7 = "\n^1- ^7Saber reflects bullets where you aim.\n";

	trap_SendServerCommand(ent - g_entities, va("print \"%s%s%s%s%s%s%s\"", s1, s2, s3, s4, s5, s6, s7));
}
