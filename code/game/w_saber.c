...
qboolean CheckSaberDamage(gentity_t *self, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate)
{
	...
	else if (self->client->ps.saberIdleWound < level.time /* Boot: */ && self->client->ps.saberMove == LS_READY)// !Boot_BlockMatchesAttack(self->client->ps.saberMove, g_entities[tr.entityNum].client->ps.saberBlocked))
		...
		tr.entityNum != self->s.number)
	{
		...
		//if (self->client->ps.saberMove == LS_A_BACK ||	//Boot - this won't be necessary
		...
		if (g_entities[tr.entityNum].client && !unblockable && WP_SaberCanBlock(&g_entities[tr.entityNum], self, tr.endpos, 0, MOD_SABER, qfalse, attackStr))
		{
			...
			//if (dmg > 5)	//Boot - won't be necessary
			//{
			...
			{
				//trap_SendServerCommand(-1, va("print \"Blocked.\n\""));
			...
			//{
			//	trap_SendServerCommand(-1, va("print \"Clashed?\n\""));
			//	g_entities[tr.entityNum].client->ps.weaponTime = 0;
			//	g_entities[tr.entityNum].client->ps.weaponstate = WEAPON_READY;
			//	g_entities[tr.entityNum].client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
		...
		{
			//if (g_entities[tr.entityNum].client && g_entities[tr.entityNum].client->ps.usingATST)	//Boot - won't be necessary
			//{
			//	dmg *= 0.1;
			//}

			//if (g_entities[tr.entityNum].client && !g_entities[tr.entityNum].client->ps.fd.forcePowerLevel[FP_SABERATTACK]) // Boot - won't be necessary
			...
			self->client->ps.saberAttackWound = level.time + 800;//100; Boot - no more double hits.
		}
	}
		...
		level.time > boot->lastSaberClashTime + 400)	//<-- Boot - don't repeat attack clashes against idle saber so often
	{ //saber clash
		gentity_t *te;
		...
		//if (self->client->ps.fd.saberAnimLevel < FORCE_LEVEL_3)	//Boot - won't be necessary. Not like this.
		//{
		//	self->client->ps.weaponTime = 0;
		//	self->client->ps.weaponstate = WEAPON_READY;
		//	self->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
		...
		//		{
		//			WP_SaberCanBlock(otherOwner, self, tr.endpos, 0, MOD_SABER, qfalse, 1);
		//		}
		//	}
		//}
		//else if (otherOwner && otherOwner->client && otherOwner->client->ps.fd.forcePowerLevel[FP_SABERDEFEND] <= self->client->ps.fd.saberAnimLevel)	//Boot - won't be necessary
		//{ //block
		//	if (otherOwner->client->ps.weaponTime < 1 || otherOwner->client->ps.fd.saberAnimLevel < FORCE_LEVEL_3)
		//	{
		//		if (WP_SaberCanBlock(otherOwner, self, tr.endpos, 0, MOD_SABER, qfalse, 1) && dmg > 5)
		//		{
		//			otherOwner->client->ps.weaponTime = 0;
		//			otherOwner->client->ps.weaponstate = WEAPON_READY;
		//			otherOwner->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
		...
		//self->client->ps.saberAttackWound = level.time + 300;	//<-- Boot. That thing saved him from getting damaged if sabers clashed.
		...
		//if (dmg > 5)	//Boot - not necessary. Yet.
		...
		//		//(otherOwner->client->ps.saberMove >= LS_A_TL2BR && self->client->ps.saberMove >= LS_A_TL2BR))	//Boot
		//	{
		//	//	WP_SaberBlockNonRandom(otherOwner, tr.endpos, qfalse);
		//		//trap_SendServerCommand(-1, va("\print \"Saberclash\n\""));
		//		/*otherOwner->client->ps.weaponTime = 0;
		//		otherOwner->client->ps.weaponstate = WEAPON_READY;
		//		otherOwner->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;*/

		//		//self->client->ps.saberBlockTime = level.time + (350 - (self->client->ps.fd.forcePowerLevel[FP_SABERDEFEND]*100));//300;
		//		//self->client->ps.weaponstate = WEAPON_READY;	//Boot
		//		//self->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
		//		//self->client->ps.weaponTime = 0;
...
int WP_SaberCanBlock(gentity_t *self, /*Boot:*/gentity_t *other, vec3_t point, int dflags, int mod, qboolean projectile, int attackStr)
{
	...
	/*if (BG_SaberInAttack(self->client->ps.saberMove))	//Boot
	{
		return 0;
	}*/

	/*if (PM_InSaberAnim(self->client->ps.torsoAnim) && !self->client->ps.saberBlocked &&	//Boot
	...
	/*if (self->client->ps.weaponstate == WEAPON_RAISING)	//Boot
	{
		return 0;
	}*/
	...
	//if ((self->client->pers.cmd.buttons & BUTTON_ATTACK)/* &&	//Boot
	...
	/*if (attackStr == FORCE_LEVEL_3)	//Boot comment
	{
	...
	//if (attackStr == FORCE_LEVEL_2 && Q_irand(1, 10) < 3)	//Boot comment
	//{
	//	if (self->client->ps.fd.forcePowerLevel[FP_SABERDEFEND] >= FORCE_LEVEL_3)
	//	{
	//		//do nothing for now
	...
	/*if (self->client->ps.saberMove != LS_READY &&	//Boot
...
qboolean Boot_BlockMatchesAttack(int attack, int block)
{
	...
	else if ((attack == LS_A_L2R || attack == LS_A_TL2BR || attack == LS_A_BL2TR) &&
		(block == BLOCKED_UPPER_RIGHT || block == BLOCKED_LOWER_RIGHT || block == BOOT_BLOCKED_DIAG_RIGHT))
	{
		return qtrue;
	}
	else if ((attack == LS_A_R2L || attack == LS_A_TR2BL || attack == LS_A_BR2TL) &&
