...
void ClientThink_real( gentity_t *ent ) {
	...
	bootSession_t *boot = &bootSession[ent - g_entities]; // Boot
...
void Boot_ManualBlockThink(gentity_t *ent)
{
	...
	if (ent->client->pers.cmd.buttons & BUTTON_MANUAL_BLOCK)
	{
		...
		if (!boot->pressingParry)
		{
			if (!boot->usesMouseMovementBlock)
			{
				...
				boot->parryDirection = Boot_GetParryAnimFromMovement(&ent->client->pers.cmd);
			}
			else
			{
	...
	// diffX = ent->client->ps.viewangles[1] - boot->lastViewAngle[1];
	// diffY = ent->client->ps.viewangles[0] - boot->lastViewAngle[0];

	// diffX *= 1000000000000;
	// diffY *= 1000000000000;
	
	boot->lerpedViewAngleDiffX = BootLerp(boot->lerpedViewAngleDiffX, ent->client->ps.viewangles[1] - boot->lastViewAngle[1], 0.6); 
	boot->lerpedViewAngleDiffY = BootLerp(boot->lerpedViewAngleDiffY, ent->client->ps.viewangles[0] - boot->lastViewAngle[0], 0.6); 

	// trap_SendServerCommand(ent - g_entities, va("print \"Horz change:%f \n\"", boot->lerpedViewAngleDiffX));
	// trap_SendServerCommand(ent - g_entities, va("print \"Vert change:%f \n\"", boot->lerpedViewAngleDiffY));
	// trap_SendServerCommand(ent - g_entities, va("print \"Horz change:%f \n\"", ent->client->ps.viewangles[1] - boot->lastViewAngle[1]));
	// trap_SendServerCommand(ent - g_entities, va("print \"Vert change:%f \n\"", ent->client->ps.viewangles[0] - boot->lastViewAngle[0]));
...
float BootLerp(float a, float b, float factor)
{
	return a + (b-a) * factor;
}
...
int Boot_GetDirFromMouseMovement(gentity_t *ent)
{
	vec3_t viewAngles;
	bootSession_t *boot = &bootSession[ent - g_entities];
	float diffX = boot->lerpedViewAngleDiffX;
	float diffY = boot->lerpedViewAngleDiffY;
